...
const orderController = {
  createOrder: async (req, res) => {
    ...
    try {
      ...
      const invalidItems = items.filter(item => !item.productId || !item.quantity || !item.price || !item.farmerId);
      if (invalidItems.length > 0) {
        console.log('❌ Invalid item data:', invalidItems);
        return res.status(400).json({ 
          message: "Each item must have productId, quantity, price, and farmerId",
          invalidItems
        });
      }

      // Validate quantities are positive numbers
      const invalidQuantities = items.filter(item => !Number.isInteger(item.quantity) || item.quantity <= 0);
